...
<body>
	...
	<table width="97%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr align="center">
			...
			if (totalRecord != 0) {//DB의 board테이블에서 조회된 글이 있다면?
				...
				if (nowBlock > 0) {
			%> <a
				href="<%=contextPath%>/Book/booktradingboard.bo?nowBlock=<%=nowBlock - 1%>&nowPage=<%=((nowBlock - 1) * pagePerBlock)%>">
					◀ 이전 <%=pagePerBlock%>개
			</a> <%
 }
 ...
 %> &nbsp;&nbsp; <a
				href="<%=contextPath%>/Book/booktradingboard.bo?nowBlock=<%=nowBlock%>&nowPage=<%=(nowBlock * pagePerBlock) + i%>">
