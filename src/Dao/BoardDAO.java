...
//			sql = "SELECT b_group, b_level FROM notice "
//				+ "WHERE notice_id=?";
//			pstmt = con.prepareStatement(sql);
//			pstmt.setInt(1, Integer.parseInt(super_notice_id));
//			rs = pstmt.executeQuery();
...
//	
////			 int b_group = rs.getInt("b_group"); // int로 가져오기
////		     int b_level = rs.getInt("b_level"); // int로 가져오기
//	
////2.		
////규칙1. 답변글을 추가하는 주글(부모글)의
////	      b_group열의 값보다 큰 값이 저장된 다른주글의 b_grop열의 값을 1증가시킨다	
//			
//	
//			sql = "UPDATE notice SET b_group = b_group + 1 WHERE b_group > '"+b_group+"'";
//			pstmt = con.prepareStatement(sql);
//			pstmt.executeUpdate();
//			
//	
...
//								  + " values(?,?,NOW(),?,?,?)";	
//			pstmt = con.prepareStatement(sql);
//			pstmt.setString(1, reply_title);//답변글 제
//			pstmt.setString(2, reply_content);//답변글 내
//			pstmt.setString(3, reply_id);//답변자 id
//			
//			//규칙2.
//			//답변글의 b_group열의 값은 주글(부모글) b_group열값에 + 1 한 값을 INSERT
//			pstmt.setInt(4, Integer.parseInt(b_group) + 1); 
//			//규칙3.
//			//답변글의 b_level열의 값은 주글(부모글) b_level열값에 + 1 한 값을 INSERT
//			pstmt.setInt(5, Integer.parseInt(b_level) + 1);
//			
//			pstmt.executeUpdate();
//			
//		} catch (Exception e) {
